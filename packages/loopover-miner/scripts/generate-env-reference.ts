...
/** One row of the generated env reference: the variable, where it is read, and its default. */
export type EnvReferenceRow = { name: string; firstReference: string; defaultValue?: string | null | undefined };
...
export function isMinerEnvVar(name: string): boolean {
  return MINER_ENV_PREFIXES.some((prefix) => name.startsWith(prefix));
}

export function collectMinerEnvDefaults(rootDir: string, sourceRoots: string[] = DEFAULT_SOURCE_ROOTS): Map<string, string> {
  ...
  for (const file of sourceFiles(rootDir, sourceRoots)) {
    ...
    for (const pattern of DEFAULT_PATTERNS) {
      ...
      for (const match of source.matchAll(pattern)) {
        const name = match[1];
        // noUncheckedIndexedAccess: a regex group can be undefined even when the match succeeded.
        if (!name || !isMinerEnvVar(name)) continue;
        const value = match[2] ?? "";
...
export function renderMinerEnvReferenceMarkdown(rows: EnvReferenceRow[]): string {
  const header = [
...
function quoteJsStringLiteral(value: string): string {
...
export function renderMinerEnvReferenceModule(rows: EnvReferenceRow[]): string {
  ...
  const markdownLines = markdown
    ...
    .join("\n");
  return `// Generated by scripts/generate-env-reference.ts (npm run miner:env-reference). Do not edit manually.
...
function sourceFiles(rootDir: string, sourceRoots: string[]): string[] {
  const files = [];
