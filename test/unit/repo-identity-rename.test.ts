...
describe("renameRepositoryIdentity", () => {
  ...
  describe("github_agent_command_feedback (#8379)", () => {
    it("renames every feedback row (no unique index on repo_full_name, so nothing folds)", async () => {
      const env = createTestEnv();
      await upsertAgentCommandAnswer(env, { id: "answer-fb", repoFullName: OLD, issueNumber: 12, command: "preflight", responseUrl: `https://github.com/${OLD}/issues/12`, actorKind: "author", metadata: {} });
