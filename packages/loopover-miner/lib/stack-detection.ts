...
type DetectedStackFields = Omit<DetectedRepoStack, "detected">;
...
const NODE_LOCKFILES: readonly [string, string][] = Object.freeze([
  ["pnpm-lock.yaml", "pnpm"],
  ["yarn.lock", "yarn"],
  ["bun.lockb", "bun"],
...
function makeAccess(repoPath: string, options: DetectRepoStackOptions): Access {
...
function nodeLockfile(exists: Access["exists"]): string | null {
...
function detectNode({ exists, read }: Access): DetectedStackFields | null {
  if (!exists("package.json")) return null;
  const pkg = parseJson(read("package.json"));
  const scripts =
    pkg && typeof pkg.scripts === "object" && pkg.scripts && !Array.isArray(pkg.scripts) ? (pkg.scripts as Record<string, unknown>) : {};
...
function detectPython({ exists, read }: Access): DetectedStackFields | null {
...
function detectRust({ exists }: Access): DetectedStackFields | null {
...
function detectGo({ exists }: Access): DetectedStackFields | null {
...
function detectMaven({ exists }: Access): DetectedStackFields | null {
...
function detectGradle({ exists }: Access): DetectedStackFields | null {
...
const DETECTORS: readonly ((access: Access) => DetectedStackFields | null)[] = Object.freeze([
  detectNode,
  detectPython,
  detectRust,
  detectGo,
...
export function renderStackSummary(stack: RepoStackResult | null | undefined): string {
