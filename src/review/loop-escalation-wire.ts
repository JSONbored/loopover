...
/** A manifest-sourced enable override (#6349 / #6275) -- the top-level `loopEscalation` block of the loopover
...
/** True when the scheduled fleet-escalation sweep is enabled. Config-as-code (#6349 / #6275): a present
 *  top-level `loopEscalation` manifest block on the loopover self-repo wins outright; otherwise falls back to
 *  the LOOPOVER_LOOP_ESCALATION env flag (default OFF). Flag-OFF (default) -> the caller never invokes it, so
 *  the cron enqueues no sweep job and the queue processor no-ops on a stale in-flight one. */
...
/**
 * Config-as-code override lookup (#6349 / #6275): read the top-level `loopEscalation` block off the loopover
 * self-repo's `.loopover.yml` (resolveLoopOverSelfRepoFullName) -- the escalation sweep is a fleet-wide,
 * operator-level cron, not a per-repo one, so ONE designated repo's manifest stands in for "the operator's own
 * config", exactly as prReconciliation / ops already do. A manifest load failure (network blip, malformed YAML)
 * degrades to `{ present: false }` -- the caller then falls through to the env var, so a hiccup can never
 * accidentally enable or disable the sweep. `nowMs` defaults to `Date.now()` so callers need no change while
 * tests can pass a deterministic value to exercise the TTL precisely. */
