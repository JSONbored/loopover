...
/** A changed file that is a statistical churn hotspot — high commit frequency AND a high fraction of
 *  fix/revert commits in recent history (#1513). Defects cluster in such files, so the reviewer scrutinizes
 *  harder. Counts + a rate only — no commit messages, authors, or dates (public-safe by construction). */
