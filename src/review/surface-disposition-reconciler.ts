...
export async function reconcileSurfaceWithoutDisposition(env: Env, nowMs: number = Date.now()): Promise<SurfaceDispositionReconcileResult> {
  ...
  let rows: Array<{ repoFullName: string; number: number; installationId: number; headSha: string; createdAt: string | null }> = [];
  try {
    const result = await env.DB.prepare(
      ...
      .all<{ repoFullName: string; number: number; installationId: number; headSha: string; createdAt: string | null }>();
    rows = result.results ?? [];
  } catch (error) {
  ...
  for (const row of rows) {
    const sent = await env.JOBS.send({
      ...
      // real PR, so this repair scan silently preempted genuinely older contributor work.
      ...(row.createdAt ? { prCreatedAt: row.createdAt } : {}),
    })
      .then(() => true)
      .catch(() => false);
    if (!sent) continue;
