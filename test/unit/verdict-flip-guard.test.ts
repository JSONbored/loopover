...
import { findingsHadAiDefect, nextVerdictFlipState, VERDICT_FLIP_ESCALATION_THRESHOLD } from "../../src/review/verdict-flip-guard";
...
describe("nextVerdictFlipState", () => {
  it("a first observation never escalates — nothing to flip against yet", () => {
    expect(nextVerdictFlipState(null, true)).toEqual({ lastHadDefect: true, flipCount: 0, escalate: false });
    expect(nextVerdictFlipState(null, false)).toEqual({ lastHadDefect: false, flipCount: 0, escalate: false });
  });

  it("repeating the same verdict does NOT add to the flip count — consistency is not the abuse pattern", () => {
    const state = { lastHadDefect: true, flipCount: 2 };
    expect(nextVerdictFlipState(state, true)).toEqual({ lastHadDefect: true, flipCount: 2, escalate: false });
  });

  it("a change from the prior verdict increments the flip count", () => {
    expect(nextVerdictFlipState({ lastHadDefect: true, flipCount: 0 }, false)).toEqual({ lastHadDefect: false, flipCount: 1, escalate: false });
  });

  it(`escalates once flipCount clears the threshold (${VERDICT_FLIP_ESCALATION_THRESHOLD})`, () => {
    const belowThreshold = { lastHadDefect: false, flipCount: VERDICT_FLIP_ESCALATION_THRESHOLD - 1 };
    expect(nextVerdictFlipState(belowThreshold, true)).toMatchObject({ flipCount: VERDICT_FLIP_ESCALATION_THRESHOLD, escalate: true });
  });

  it("simulates the exploit: defect, clean, defect, clean — escalates on the Nth flip, not before", () => {
    let state: { lastHadDefect: boolean; flipCount: number } | null = null;
    const rolls = [true, false, true, false, true]; // 4 flips across 5 rolls
    const escalations: boolean[] = [];
    for (const hadDefect of rolls) {
      const next = nextVerdictFlipState(state, hadDefect);
...
describe("verdict-flip store (D1, fail-open)", () => {
  ...
  it("recordVerdictFlip: persists across calls, escalates at the threshold, and a write error still returns the computed (unpersisted) state", async () => {
    ...
    expect(await recordVerdictFlip(env, "o/r", 5, defect)).toMatchObject({ lastHadDefect: true, flipCount: 0, escalate: false });
    // Rolls 2-4: clean, defect, clean — 3 flips, escalates on the 3rd.
    expect(await recordVerdictFlip(env, "o/r", 5, clean)).toMatchObject({ flipCount: 1, escalate: false });
    expect(await recordVerdictFlip(env, "o/r", 5, defect)).toMatchObject({ flipCount: 2, escalate: false });
    expect(await recordVerdictFlip(env, "o/r", 5, clean)).toMatchObject({ flipCount: 3, escalate: true });
    // State persisted between calls (a different PR is fully independent).
    expect(await readVerdictFlipState(env, "o/r", 5)).toEqual({ lastHadDefect: false, flipCount: 3 });
    ...
    await recordVerdictFlip(writeBroken, "o/r", 6, defect);
    vi.spyOn(writeBroken.DB, "prepare").mockImplementation(() => {
      throw new Error("db down");
    });
    const result = await recordVerdictFlip(writeBroken, "o/r", 6, clean);
