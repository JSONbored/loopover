...
registerStdioTool("loopover_plan_idea_claims", {
    ...
    if (!result.ok)
        return toolResult(`Invalid idea submission: ${result.errors.join(", ")}.`, result);
    const { claimPlan } = result;
    return toolResult(`Claim plan: ${claimPlan.claimable.length} claimable, ${claimPlan.deferred.length} deferred, ${claimPlan.skipped.length} skipped.`, result);
});
