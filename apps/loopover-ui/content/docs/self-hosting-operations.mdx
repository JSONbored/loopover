...
# Optional: per-repo/global severity floors (default: only error/critical reach PostHog)
...
<FeatureRow
  items={[
    {
      title: "Operational properties (low cardinality)",
      description:
        "repo (also set when logs emit repository), pr, head_sha, kind, subsystem, jobType, operation, agent, decision_outcome, provider, model, monitor, installation_id_hash (never raw installation id), trace_id, span_id.",
    },
    {
      title: "Full scrubbed detail",
      description:
        "The rest of the captured context (review failures, structured console lines) is merged into the same properties bag after redaction. Secrets, tokens, bodies, diffs, prompts, and review text are redacted before send.",
    },
    {
      title: "Subsystems",
      description:
  ...
  <table className="w-full border-collapse text-token-sm">
    <thead>
      <tr className="border-hairline text-left text-token-xs text-muted-foreground">
        ...
        <th className="py-2 pr-4 font-medium">PostHog signal</th>
    ...
    <tbody className="divide-hairline">
      ...
      <tr>
        ...
        <td className="py-2 pr-4 align-top text-muted-foreground">
          PostHog insight alert on a no-data condition over <code>orb_monitor_heartbeat</code>,
          filtered to <code>scheduled-loop</code>, <code>orb-export</code>, or other wrapped loops
        </td>
        <td className="py-2 pr-4 align-top text-muted-foreground">
...
<CodeBlock
  filename=".env"
  code={`# Turn tracing on
OTEL_TRACES_EXPORTER=otlp
# If POSTHOG_API_KEY (above) is also set and you leave the two vars below unset, traces default
# to PostHog's own distributed-tracing product (https://posthog.com/docs/distributed-tracing,
# currently in beta) automatically -- same project, same auth, no extra config.
#
# To send traces somewhere else instead (your own OTel collector, another vendor), set an
# explicit endpoint -- it always wins over the PostHog default:
