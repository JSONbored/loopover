...
describe("evaluateTenantQuotaSoftWarnings (#7662)", () => {
  ...
  it("reports each low-but-allowed dimension independently", () => {
  ...
  it("skips dimensions with a zero cap", () => {
    const decision = evaluateTenantQuota(
      { computeUnitsUsed: 0, wallClockMsUsed: 59_500, activeLoops: 0 },
      { computeUnits: 0, wallClockMs: 60_000, maxConcurrentLoops: 3 },
    );
    expect(evaluateTenantQuotaSoftWarnings(decision, { computeUnits: 0, wallClockMs: 60_000, maxConcurrentLoops: 3 })).toEqual([
      { dimension: "time", severity: "critical", remaining: 500, cap: 60_000 },
    ]);
  });
});
