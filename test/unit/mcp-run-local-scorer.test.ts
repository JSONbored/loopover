...
async function connect() {
  ...
  const client = new Client({ name: "loopover-scorer-test", version: "0.1.0" }, { capabilities: {} });
...
describe("MCP loopover_run_local_scorer (#782)", () => {
  it("returns deterministic token scores from changed-file metadata (no repo/auth needed)", async () => {
    ...
    expect(data.tokenScores).toMatchObject({ mode: "external_command", sourceTokenScore: 12, testTokenScore: 8, nonCodeTokenScore: 5, totalTokenScore: 17.4 }); // #8875: 12 + 0.05 × 8 + 5
