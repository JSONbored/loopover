...
test("detectErrorSwallow: a `return null` that is one branch of a body that ALSO rethrows is not return-null", () => {
  // return-null requires the WHOLE body to be the null return; a guarded null-return that then rethrows is
  // real error handling, not a swallow.
  assert.equal(detectErrorSwallow("} catch (e) { if (recoverable) return null; throw e; }", "js"), null);
  assert.equal(detectErrorSwallow("} catch (e) { log(e); return null; }", "js"), null); // logs, then returns
});

test("detectErrorSwallow: a binding containing `$`/`_` is matched safely (regex-escaped, identifier boundary)", () => {
  // A `$`-prefixed binding must not break the reference regex, and a real reference to it must be recognized.
  assert.equal(detectErrorSwallow("} catch ($err) { report($err); }", "js"), null); // references $err
  assert.equal(detectErrorSwallow("} catch (_e) { use(_e.stack); }", "js"), null); // references _e
  // …but a body that ignores the `$`-binding is still an unused-binding.
  assert.equal(detectErrorSwallow("} catch ($err) { cleanup(); }", "js"), "unused-binding");
});
