...
describe("loopover-miner claim ledger expiry (#2316)", () => {
  ...
  it("findExpiredClaims sweeps an active claim whose claimedAt is unparseable, fail-closed (NaN age path) (#7732)", () => {
    ...
    const fresh = claim({ issueNumber: 9, claimedAt: "2026-07-02T18:00:00.000Z" }); // in-window, must stay
    // #7732: a corrupted/hand-edited row with a null age must be swept (fail-closed) rather than left
    // permanently un-expirable; a valid in-window claim is still untouched.
