    ...
    const driver = createAgentSdkCodingAgentDriver({
      query: () =>
        (async function* (): AsyncGenerator<Record<string, unknown>> {
          ...
          throw new Error("bridge died: token sk-abcdefghijklmnop1234 leaked");
        })(),
    });
    const result = await driver.run(task);
    expect(result.ok).toBe(false);
    expect(result.error).toMatch(/^agent_sdk_thrown: bridge died/);
    expect(result.error).not.toContain("sk-abcdefghijklmnop1234");
      ...
      query: queryYielding([
        {
          ...
          is_error: false,
          num_turns: 2,
          result: "done, but echoed ghp_abcdefghijklmnopqrst123456",
        },
      ]),
    });
    const result = await driver.run(task);
    expect(result.ok).toBe(true);
    expect(result.summary).not.toContain("ghp_abcdefghijklmnopqrst123456");
    expect(result.summary).toContain("[redacted]");
    expect(result.transcript).not.toContain("ghp_abcdefghijklmnopqrst123456");
  });
