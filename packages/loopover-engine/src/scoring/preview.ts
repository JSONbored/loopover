...
import { labelPatternToRegExp } from "./label-match.js";
...
// `src/utils/json.ts`. Label-pattern matching (`labelMatchesPattern`/`labelPatternToRegExp`) and its
// compiled-regex cache live once in `./label-match.ts` — the module extracted for exactly this reuse
// (#7254) — and are imported/re-exported here rather than duplicated.
...
// #7254: labelMatchesPattern, the compiled-regex LRU cache-size constant, and the cache test helpers are
// the public surface consumers import from preview (signals/engine.ts, src/rules/advisory.ts, and the
// src/scoring/preview re-export shim that test/unit/scoring.test.ts imports). Re-export them from the
// shared ./label-match.ts module rather than maintaining a second, drift-prone identical copy here.
export {
  labelMatchesPattern,
