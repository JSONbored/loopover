...
describe("runMinerAttempt (#2337) — the real create->review->gate->submit pipeline", () => {
  ...
  it("REGRESSION (#5676): a near-duplicate of a real recent own submission is throttled at the self-plagiarism stage", async () => {
    ...
    deps.governorState.recordOwnSubmission({
      ...
      // Earlier than the prospective submission (its submittedAt is new Date(deps.nowMs)), so THIS attempt is the
      // later near-duplicate the self-plagiarism throttle denies -- matching self-plagiarism.ts's "the later of two
      // near-identical submissions is the plagiarist" tie-break.
