...
export interface OrbExportRow { repoHash: string; prHash: string; decision: string; reasonBucket: string; closedAt: string | null; }
export interface OrbExportStore { dbPath: string; getOrCreateAnonSecret(): string; getCursor(): string | null; setCursor(cursor: string): void; close(): void; }
...
type RunOrbExportCliOptions = { openOrbExportStore?: () => OrbExportStore; initEventLedger?: () => PrOutcomeLedgerReader & { close(): void }; sendAmsExportBatch?: (options: { batch: OrbExportRow[]; secret: string; collectorToken?: string | undefined }) => Promise<AmsExportSendResult>; env?: Record<string, string | undefined> };
 ...
 */
export function buildAnonymizedOrbBatch(outcomes: Iterable<OrbExportOutcome> | Map<string, OrbExportOutcome>, secret: string): OrbExportRow[] {
  const iterable = outcomes && typeof (outcomes as Map<string, OrbExportOutcome>).values === "function" ? (outcomes as Map<string, OrbExportOutcome>).values() : outcomes as Iterable<OrbExportOutcome>;
 ...
 */
export function collectOrbExportBatch({ store, eventLedger, enabled = ORB_EXPORT_ENABLED_BY_DEFAULT }: { store?: OrbExportStore; eventLedger?: PrOutcomeLedgerReader; enabled?: boolean } = {}): OrbExportRow[] | null {
...
}: { batch: OrbExportRow[]; secret: string; collectorUrl?: string; collectorToken?: string | undefined; fetchFn?: typeof fetch; timeoutMs?: number }): Promise<AmsExportSendResult> {
 ...
 *  already delivered. */
export async function runOrbExportCli(args: string[], options: RunOrbExportCliOptions = {}): Promise<number> {
