...
import {
  applySelfHostFetchTrustedClientIp,
...
describe("trusted-client-ip (self-host rate-limit identity)", () => {
  ...
  it("applySelfHostFetchTrustedClientIp mirrors the server.ts serve({ fetch }) edge wiring", () => {
    const original = new Request("https://orb.example/v1/auth/github/session", {
      headers: {
        "cf-connecting-ip": "203.0.113.1",
    ...
    const trusted = applySelfHostFetchTrustedClientIp(original, {
      incoming: { socket: { remoteAddress: "10.0.0.5" } },
    });
    expect(trusted.headers.get("cf-connecting-ip")).toBe("198.51.100.88");
  });
  ...
  it("accepts bracketed IPv6 peers/headers and rejects malformed IPv4/IPv6", () => {
    expect(resolveTrustedClientIp("[2001:db8::1]", new Headers())).toBe("2001:db8::1");
    expect(
