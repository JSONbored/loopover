  ...
  return {
    async run(task: CodingAgentDriverTask): Promise<CodingAgentDriverResult> {
      ...
      try {
        ...
        const detail = redactSecrets(error instanceof Error ? error.message : String(error)).slice(0, 500);
        return {
          ok: false,
          changedFiles: [],
      ...
      return {
        ...
        summary: resultText.slice(0, 500) || `coding agent completed with ${changedFiles.size} changed file(s)`,
        transcript,
        turnsUsed,
      };
    },
