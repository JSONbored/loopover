...
describe("queue processors", () => {
  ...
  it("loop-escalation-sweep job no-ops when a present loopEscalation manifest override turns it OFF even though LOOPOVER_LOOP_ESCALATION is ON (#8018)", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const env = createTestEnv({
      LOOPOVER_LOOP_ESCALATION: "true",
    ...
    await upsertRepoFocusManifest(env, "JSONbored/gittensory", { loopEscalation: { enabled: false } });
    await processJob(env, { type: "loop-escalation-sweep", requestedBy: "test" });
    expect(errorSpy.mock.calls.map((c) => String(c[0])).some((line) => line.includes("loop_escalation_needs_attention"))).toBe(false);
    errorSpy.mockRestore();
  });

  it("loop-escalation-sweep job runs when a present loopEscalation manifest override turns it ON even though LOOPOVER_LOOP_ESCALATION is OFF (#8018)", async () => {
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const env = createTestEnv({
      LOOPOVER_DRIFT_ISSUE_REPO: "JSONbored/gittensory",
      LOOPOVER_ACTIVE_LOOPS_JSON: JSON.stringify([{ loopId: "broken", tenantId: "acme", runStatus: "abandoned" }]),
    });
    await upsertRepoFocusManifest(env, "JSONbored/gittensory", { loopEscalation: { enabled: true } });
    await processJob(env, { type: "loop-escalation-sweep", requestedBy: "test" });
    expect(errorSpy.mock.calls.map((c) => String(c[0])).some((line) => line.includes("loop_escalation_needs_attention"))).toBe(true);
    errorSpy.mockRestore();
  });
