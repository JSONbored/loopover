...
export function readInstalledEnginePackageVersionFromPaths(
  ...
  workspacePkg: string,
  deps: { existsSync: (path: string) => boolean; readFileSync: (path: string, encoding: "utf8") => string } = { existsSync, readFileSync },
): string | null {
  try {
...
export function compareInstalledEngineVersion(installed: string, expected: string): -1 | 0 | 1 {
  ...
  for (let index = 0; index < 3; index += 1) {
    const installedPart = installedCore[index] as number;
    const expectedPart = expectedCore[index] as number;
    if (installedPart < expectedPart) return -1;
    if (installedPart > expectedPart) return 1;
  }
  return 0;
}
...
function resolveDriverStatus(env: Record<string, string | undefined>): MinerDriverStatus {
  ...
  const modelEnvVar = provider ? (CODING_AGENT_DRIVER_CONFIG_ENV[provider as keyof typeof CODING_AGENT_DRIVER_CONFIG_ENV]?.model ?? null) : null;
