...
// A single changed-file entry in a local diff. `path` is always present; the stat fields are absent
// for a binary file (git reports `-`/`-`) or an unparseable numstat row and are filled in as 0.
...
// The MCP roots a client exposes, normalized to local filesystem paths (see normalizeMcpWorkspaceRoots).
export type McpWorkspaceRoot = { path: string };

// Caller-supplied branch context. Every field is optional: whatever is not passed is derived from the
// local git checkout. Kept open (no exhaustive enumeration of the pass-through scenario fields) because
// the bin forwards a superset straight through to the analysis payload.
...
// The external scorer runs as a separate process and its status/diagnostic bag varies by failure mode
// (see scorerFailure / classifyScorerExecFailure), so both the status and the scorer's own JSON stay
// open records rather than a closed shape that every branch would have to satisfy exactly.
type ScorerStatus = Record<string, unknown>;
...
export function collectLocalDiff(cwd: string, baseRef: string | undefined, workspaceRoots: unknown) {
...
export function resolveWorkspaceCwd(input: LocalBranchInput = {}) {
...
export function resolveScorePreviewCommand(input: LocalBranchInput = {}): string | undefined {
...
export function sanitizeLocalScorerStatus(status: unknown): unknown {
  ...
  const bag = status as ScorerStatus;
...
export function setupGuidanceForLocalScorer(status: ScorerStatus): string[] {
  if (status.ok) return [];
  const safeStatus = sanitizeLocalScorerStatus(status) as ScorerStatus;
...
export function probeLocalScorer(scorerCommand: string | undefined = resolveScorePreviewCommand()): unknown {
...
function collectChangedFiles(cwd: string, baseRef: string): ChangedFile[] {
  ...
  const numstat = new Map(parseNumstat(cwd, baseRef).map((entry) => [entry.path, entry] as const));
...
function parseNameStatus(cwd: string, baseRef: string) {
...
function parseNumstat(cwd: string, baseRef: string) {
...
function normalizeScorerOutput(payload: ScorerPayload) {
  const source = payload.source as ScorerPayload | undefined;
  const total = payload.total as ScorerPayload | undefined;
  const tests = payload.tests as ScorerPayload | undefined;
  const nonCode = payload.nonCode as ScorerPayload | undefined;
  return stripUndefined({
...
function metadataOnlyScorer(status: ScorerStatus) {
  return {
    mode: "metadata_only",
...
function scorerFailure(code: string, reason: string, extra: Record<string, unknown> = {}): ScorerStatus {
  return stripUndefined({
    ok: false,
    code,
    reason,
...
function classifyScorerExecFailure(error: unknown, durationMs: number, scorerCommand: string | undefined): ScorerStatus {
  const execError: any = error && typeof error === "object" ? error : undefined;
...
function titleFromBranch(branchName: unknown): string {
...
function stripUndefined<T>(value: T): T {
  ...
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).filter(([, entry]) => entry !== undefined).map(([key, entry]) => [key, stripUndefined(entry)])) as T;
}
