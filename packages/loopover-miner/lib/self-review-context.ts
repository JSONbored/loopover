...
import type { FocusManifest, IssueRecord, PullRequestRecord, RepositoryRecord, SelfReviewContext } from "@loopover/engine";
...
// LiveIssueSnapshotFetch), but a bit richer than the shared narrow shape other modules use: this module's
// bounded manifest reader also needs `headers.get()` (content-length bound) and a streaming `body.getReader()`
// (bounded read without loading an oversized response fully into memory first) -- both OPTIONAL here so a
// caller that never supplies them (the plain-JSON GitHub/Gittensor call sites) still satisfies it.
...
type SelfReviewContextResponse = Awaited<ReturnType<SelfReviewContextFetch>>;
...
const resolveBackendSession = resolveLoopoverBackendSession as (
  env?: Record<string, string | undefined>,
) => LoopoverBackendSessionAuth | null;

function normalizeOptions(options: FetchSelfReviewContextOptions = {}): ResolvedOptions {
  const env = options.env ?? process.env;
  // Explicit null skips the probe (tests / forced-standalone). Undefined ⇒ resolve from loopover-mcp session.
  const loopoverAuth: LoopoverBackendSessionAuth | null =
  ...
  return {
    ...
    fetchImpl: options.fetchImpl ?? (fetch as unknown as SelfReviewContextFetch),
...
async function probeLiveGateThresholds(target: { owner: string; repo: string }, resolved: ResolvedOptions): Promise<LiveGateThresholdFields | null> {
...
async function fetchWithTimeout(
  ...
  timeoutMs: number,
): Promise<SelfReviewContextResponse> {
  return fetchImpl(url, { ...init, signal: AbortSignal.timeout(timeoutMs) });
}

async function githubGetJson(url: string, resolved: ResolvedOptions): Promise<{ response: SelfReviewContextResponse; payload: unknown }> {
...
async function fetchPaginated(pathWithQuery: string, resolved: ResolvedOptions): Promise<unknown[]> {
...
async function fetchRepositoryRecord(target: { owner: string; repo: string }, resolved: ResolvedOptions): Promise<RepositoryRecord | null> {
  ...
  const record = payload as { owner?: { login?: unknown }; name?: unknown; private?: unknown; html_url?: unknown; default_branch?: unknown };
  return {
    fullName: `${target.owner}/${target.repo}`,
    owner: (record.owner?.login as string | undefined) ?? target.owner,
    name: (record.name as string | undefined) ?? target.repo,
    installationId: undefined,
    isInstalled: false,
    isRegistered: false,
    isPrivate: (record.private as boolean | undefined) ?? false,
    htmlUrl: (record.html_url as string | undefined) ?? null,
...
function labelNames(labels: unknown): string[] {
  ...
  return labels.flatMap((label) => (label && typeof label === "object" && typeof (label as { name?: unknown }).name === "string" ? [(label as { name: string }).name] : []));
}
...
function toIssueRecord(repoFullName: string, issue: Record<string, unknown>): IssueRecord {
  const body = (issue.body as string | undefined) ?? "";
  const user = issue.user as { login?: unknown } | undefined;
  return {
    repoFullName,
    ...
    authorAssociation: (issue.author_association as string | undefined) ?? null,
    htmlUrl: (issue.html_url as string | undefined) ?? null,
    body,
    createdAt: (issue.created_at as string | undefined) ?? null,
    updatedAt: (issue.updated_at as string | undefined) ?? null,
    closedAt: (issue.closed_at as string | undefined) ?? null,
    labels: labelNames(issue.labels),
    linkedPrs: extractLinkedPrNumbers(body),
  };
}

async function fetchOpenIssueRecords(target: { owner: string; repo: string }, resolved: ResolvedOptions): Promise<IssueRecord[]> {
  ...
  return payloads
    .filter((issue): issue is Record<string, unknown> => Boolean(issue) && typeof issue === "object" && !(issue as { pull_request?: unknown }).pull_request)
    .map((issue) => toIssueRecord(`${target.owner}/${target.repo}`, issue));
}

function mergeableBooleanState(mergeable: unknown): string | null {
...
function toPullRequestRecord(repoFullName: string, pr: Record<string, unknown>): PullRequestRecord {
  const body = (pr.body as string | undefined) ?? "";
  const user = pr.user as { login?: unknown } | undefined;
  const head = pr.head as { sha?: unknown; ref?: unknown } | undefined;
  ...
  return {
    ...
    authorLogin: (user?.login as string | undefined) ?? null,
    authorAssociation: (pr.author_association as string | undefined) ?? null,
    ...
    htmlUrl: (pr.html_url as string | undefined) ?? null,
    mergedAt: (pr.merged_at as string | undefined) ?? null,
    isDraft: (pr.draft as boolean | undefined) ?? null,
    mergeableState: (pr.mergeable_state as string | undefined) ?? mergeableBooleanState(pr.mergeable),
    reviewDecision: null,
    body,
    createdAt: (pr.created_at as string | undefined) ?? null,
    updatedAt: (pr.updated_at as string | undefined) ?? null,
    closedAt: (pr.closed_at as string | undefined) ?? null,
    labels: labelNames(pr.labels),
    linkedIssues: extractLinkedIssueNumbers(body, repoFullName),
  };
}

async function fetchOpenPullRequestRecords(target: { owner: string; repo: string }, resolved: ResolvedOptions): Promise<PullRequestRecord[]> {
  const payloads = await fetchPaginated(
    `/repos/${encodeURIComponent(target.owner)}/${encodeURIComponent(target.repo)}/pulls?state=open&sort=created&direction=asc`,
    resolved,
  );
  return (payloads as Record<string, unknown>[]).map((pr) => toPullRequestRecord(`${target.owner}/${target.repo}`, pr));
}
...
async function readBoundedManifestResponseText(response: SelfReviewContextResponse): Promise<string | null> {
...
async function fetchManifestContent(target: { owner: string; repo: string }, resolved: ResolvedOptions): Promise<string | null> {
...
async function fetchConfirmedContributor(login: string, resolved: ResolvedOptions): Promise<boolean> {
  ...
  try {
    ...
    return payload.some((miner) => typeof (miner as { githubUsername?: unknown })?.githubUsername === "string" && (miner as { githubUsername: string }).githubUsername.toLowerCase() === normalizedLogin);
  } catch {
    return false;
  }
}
