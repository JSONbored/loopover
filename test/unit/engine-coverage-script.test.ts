...
import { normalizeLcovSfPaths } from "../../scripts/engine-coverage.mjs";
...
describe("engine-coverage script", () => {
  describe("normalizeLcovSfPaths", () => {
    ...
    it("re-throws a write failure instead of masking it as a missing report", () => {
      ...
      const readFile = vi.fn(() => "SF:packages\\loopover-engine\\src\\foo.ts\nend_of_record\n");
      const writeFile = vi.fn(() => {
        throw writeErr;
      });
    ...
    it("normalizes backslashes in SF: paths to forward slashes", () => {
      ...
      expect(written).toBe("SF:packages/loopover-engine/src/foo.ts\nend_of_record\n");
    });
  });
});
