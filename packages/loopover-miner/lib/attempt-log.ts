...
import type { AttemptLogEvent, NormalizedAttemptLogEvent } from "@loopover/engine";
...
function normalizeReadAttemptIdFilter(attemptId: string | null | undefined): string | undefined {
...
function normalizeRequiredAttemptId(attemptId: string): string {
...
function ensureOutcomeColumns(db: DatabaseSync): void {
  const existingColumns = new Set(
    db.prepare("PRAGMA table_info(attempt_log_events)").all().map((column) => column.name as string),
  );
  ...
  return {
    ...
    appendAttemptLogEvent(event: AttemptLogEvent): AttemptLogEntry {
      ...
      try {
        ...
        const entry = rowToEntry(getByIdStatement.get(Number(result.lastInsertRowid)) as unknown as AttemptLogRow);
        db.exec("COMMIT");
        return entry;
      } catch (error) {
        db.exec("ROLLBACK");
    ...
    readAttemptLogEvents(filter: ReadAttemptLogEventsFilter = {}): AttemptLogEntry[] {
      ...
      return (rows as unknown as AttemptLogRow[]).map(rowToEntry);
    },
    exportAttemptLogJsonl(attemptId: string): string {
      const scopedAttemptId = normalizeRequiredAttemptId(attemptId);
      const rows = readByAttemptStatement.all(scopedAttemptId) as unknown as AttemptLogRow[];
