...
function githubError(response: Response, payload: unknown): Error & { code: string; githubMessage: string | null } {
...
async function githubGetJsonResponse(url: string, options: NormalizedPollOptions): Promise<{ payload: unknown; response: Response }> {
  ...
  const response = await fetchWithRetry(
    options.fetchFn as unknown as (url: unknown, init?: unknown) => Promise<Response>,
    url,
...
function payloadTotalCount(payload: unknown): number | null {
  const record = payload as { total_count?: unknown } | null;
...
function normalizeCheckRun(checkRun: unknown): NormalizedCheckRun {
  const record = checkRun as { name?: unknown; status?: unknown; details_url?: unknown; started_at?: unknown; completed_at?: unknown } | null;
  return {
...
async function fetchHeadSha(target: { owner: string; repo: string }, prNumber: number, options: NormalizedPollOptions): Promise<string> {
  const payload = await githubGetJson(
    apiUrl(options.apiBaseUrl, repoPath(target, `/pulls/${prNumber}`)),
    options,
  );
  const record = payload as { head?: { sha?: unknown } } | null;
  const headSha = record?.head?.sha;
  if (typeof headSha !== "string" || !headSha) throw new Error("github_pr_head_sha_missing");
  return headSha;
}

async function fetchCheckRuns(target: { owner: string; repo: string }, headSha: string, options: NormalizedPollOptions): Promise<NormalizedCheckRun[]> {
  ...
  for (;;) {
    ...
    const record = payload as { check_runs?: unknown } | null;
...
export async function pollCheckRuns(repoFullName: string, prNumber: number, options: PollCheckRunsOptions = {}): Promise<PollCheckRunsResult> {
